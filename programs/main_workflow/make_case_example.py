...
def case_example_dict(sceneid, case_example_es, u_TG_matrix, p_charge_matrix, p_discharge_matrix):
    # 提取相关参数
    T = case_example_es['T']
    ...
    ES_bl = case_example_es['ES_bl']
    ES_ramp = case_example_es['ES_ramp'].astype(float)
    ES_offer = case_example_es['ES_offer'].astype(float)
    ES_P = case_example_es['ES_P'].astype(float)

    # 更新参数
    RG_num = RG_num + ES_num
    D_num = D_num + ES_num

    RG_bl = np.hstack([RG_bl, ES_bl])
    RG_offer = np.hstack([RG_offer, ES_offer])
    RG_P = np.hstack([RG_P, ES_ramp])
    RG_ramp = np.hstack([RG_ramp, ES_ramp])
    RG_cap = np.hstack([RG_cap, p_discharge_matrix])

    D_bl = np.hstack([D_bl, ES_bl])
    D_P = np.hstack([D_P_base, p_charge_matrix])
    ...
    class case_example:
        def __init__(self):
            ...
            self.u_TG = np.array(u_TG_matrix)
    ...
    print(f"已将所有预处理数据保存到 case_example_dict_{sceneid}.pkl")
