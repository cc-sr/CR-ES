...
def PTDF(file_path):
    ...
    PTDF_matrix = np.matmul(X_line, B_line_inv)

    # 剔除 PTDF 中所有值接近 0 的行
    for row_index in range(PTDF_matrix.shape[0]):
        if np.all(np.abs(PTDF_matrix[row_index, :]) < 0.0001):
            case_example['branch'][row_index, 3] = 0

    # 找出需要进行潮流约束的 branch
    branch_check_id = np.nonzero(case_example['branch'][:, 3])[0]
    PTDF_matrix = PTDF_matrix[branch_check_id, :]
