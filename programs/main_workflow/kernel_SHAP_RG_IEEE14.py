...
def kernel_SHAP(t, sceneid, fai_all, agents_num, ES_num, kernel_num, rand_num, all_An, all_bn, all_time):
    ...
    for randid in range(rand_num):
        ...
        for kernel_id in range(para_num):
            ...
            # 合并列
            SHAP_t = np.hstack((
                SHAP_origin_reshape[:, 0:5],
                SHAP_origin_reshape[:, 7:18],
                (SHAP_origin_reshape[:, 5] + SHAP_origin_reshape[:, 18]).reshape(-1, 1),  # 第5列 + 第18列
                (SHAP_origin_reshape[:, 6] + SHAP_origin_reshape[:, 19]).reshape(-1, 1)   # 第6列 + 第19列
            ))
